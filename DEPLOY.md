...
- **Two responses to every Telegram command (one “old” behaviour)**  
  - Two processes are using the same bot token (both receive every update and both reply). The deploy workflow runs `docker compose down` then `up -d --build` so only one set of containers runs. If you still see duplicates: (1) On the VPS run `docker ps -a` and look for more than one container for the same service (e.g. two `messenger`). (2) Run `docker compose down` in the app directory, then `docker compose up -d --build`, so any stray containers are removed. (3) Check you’re not running the same stack from another directory or with another compose project name.
